...
- Registers CodeQL analysis against the immutable `refs/pull/<number>/head` identity, pins every remote GitHub Action to a full commit SHA, and adds a repository policy check that prevents ordinary branch-ref and moving-merge-ref registration drift.
...
- CodeQL checks out the exact pull-request head SHA and registers results as `refs/pull/<number>/head`. The repository ruleset can therefore approve a GitHub-generated, verified squash commit from the analyzed source without requiring an unsigned source commit on `main`.
- GitHub's Code Scanning API was used during Alpha.4 preparation to verify that successful Rust, JavaScript/TypeScript, and GitHub Actions analyses had empty error fields and nonzero rule counts under the PR-head identity; the temporary diagnostic workflow was then removed.
...
- Repository-owned policy tests reject unpinned remote Actions, ordinary branch refs or moving PR merge refs for CodeQL registration, pull-request path filters that could suppress the required product gate, and stable-release markers in the prerelease workflow.
