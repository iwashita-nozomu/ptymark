...
fn filtered_run_session_modes_share_the_runtime_factory() {
    for (mode, should_render) in [("--source", false), ("--safe", false), ("--private", true)] {
        let mut child = Command::new(binary())
            ...
            .stdout(Stdio::piped())
            .spawn()
            .expect("spawn filtered run mode");
        child
            .stdin
            .take()
            .expect("stdin")
        ...
        let text = String::from_utf8(output.stdout).expect("UTF-8");
        assert_eq!(text.contains("ptymark math"), should_render, "mode={mode}");
        assert_eq!(text.contains("$$"), !should_render, "mode={mode}");
    }
}
...
fn top_level_and_run_help_describe_the_filtered_path() {
    ...
    let text = String::from_utf8_lossy(&run_help.stdout);
    assert!(text.contains("stdout is a pipe, not a PTY"));
