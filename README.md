...
`ptymark`は、子プロセスの端末出力を**表示前**に受け取り、明確に閉じた
Markdown系の意味ブロックだけを既存レンダリングエンジンへ渡して、端末向け表示へ
差し替えるpre-display rendererです。
...
ptymark pre-display renderer
    ...
    ├─ ```mermaid ... ```          → Mermaid engine adapter
    └─ $$ ... $$                    → TeX engine adapter
    ↓ display bytes
terminal emulator
```
...
fallback、既存エンジンの起動、将来のPTY proxyはRustコアに置き、WezTerm以外からも
同じバイナリを使える設計にします。
...
- 外部既存エンジン用のbounded stdio adapter
...
- Mermaid CLI / KaTeX / Typst adapterのCLI既定選択
- terminal image protocolへの配置
- live resize generation、画像再配置、persistent cache

そのため、`ptymark -- zsh -l`は現在は公開CLI形を固定する透過`exec`です。
`ptymark preview`は表示前レンダラーの実装とテストに使えます。未実装のruntime作業は
...
有効にした利用者だけが該当エンジンを導入します。初期のrelease artifactとchecksumは
`.github/workflows/ptymark-release.yml`で生成します。
...
repository全体の開発・正式検証はDocker必須ですが、利用者向けネイティブbinaryの
ビルド自体はCargoで行えます。
...
ローカルplugin開発ではHTTPS URLの代わりに使います。
...
renderer failureをsource fallbackではなくエラーにする開発用モード:
...
現在の`ExternalRenderer`は、既存エンジンwrapperへbodyをstdinで渡し、stdoutを
artifactとして受け取るbounded adapterです。Docker checkでMermaid SVG、KaTeX
MathML、Typst SVGを実際に生成します。
