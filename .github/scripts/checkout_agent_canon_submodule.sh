...
# GitHub Actions applies GITHUB_ENV changes only to later steps. Blank inherited
# credentials there even when a workflow declared them at workflow scope.
if [ -n "${GITHUB_ENV:-}" ]; then
  {
    echo 'AGENT_CANON_REPO_TOKEN='
    echo 'AGENT_CANON_REPO_SSH_KEY='
...
# Public submodules must not use repository credentials. Only fall back to the
# command-scoped private authentication paths when anonymous access fails.
...
If this is an untrusted PR context, repository secrets may be intentionally
