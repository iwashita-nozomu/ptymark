...
fn load_matching_install_state(config_path: &Path, user: &UserConfig) -> Option<InstallState> {
    let state_path = default_install_state_path().ok()?;
    let state = InstallState::load(&state_path).ok()?;
    state
        .matches_user_config(config_path, user)
        .then_some(state)
}
    ...
    fn into_user_config(self) -> UserConfig {
        ...
        let profile = ProfileConfig {
            ...
            presentation: UserPresentationConfig {
                ...
                color: ColorPolicy::Auto,
                fallback_columns: self.rendering.columns.clamp(1, MAX_FALLBACK_COLUMNS),
            },
